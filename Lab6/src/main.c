...
//enum Exam { STKU, ASP, STU, OTS, EMPIV, TAS };
...
int main()
{
	...
	rb.lastName = "Янкина";
	rb.firstName = "Александра";
	rb.middleName = "Дмитриевна";
	rb.number = 17184;

	printf("Зачётная книжка №%d\n", rb.number);
	printf("ФИО студента: %s %s %s\n\n", rb.lastName, rb.firstName, rb.middleName);
	
	rb.bookList[0][0].exam = "ФИЗИКА"; rb.bookList[0][0].hasPassed = "ЗАЧЁТ"; rb.bookList[0][0].mark = B;
	rb.bookList[0][1].exam = "АСП"; rb.bookList[0][1].hasPassed = "ЗАЧЁТ"; rb.bookList[0][1].mark = A;

	rb.bookList[1][0].exam = "ОТС"; rb.bookList[1][0].hasPassed = "ЗАЧЁТ"; rb.bookList[1][0].mark = C;
	rb.bookList[1][1].exam = "ТЭЦ"; rb.bookList[1][1].hasPassed = "ЗАЧЁТ"; rb.bookList[1][1].mark = A;

	rb.bookList[2][0].exam = "СТКУ"; rb.bookList[2][0].hasPassed = "ЗАЧЁТ"; rb.bookList[2][0].mark = B;
